...
bool CC1muXp0pi::Selection(AnalysisEvent* Event) {
  ...
  muon_py.clear();
  muon_pz.clear();
  std::vector<int> proton_index;
  std::vector<double> proton_pid_score, proton_e, proton_px, proton_py, proton_pz;
  proton_index.clear();
  ...
  for ( int p = 0; p < Event->num_pf_particles_; ++p ) {
    ...
    if ( p == muon_candidate_idx_ ) {
      ...
      float track_score = Event->pfp_track_score_->at( p );
      if ( track_score <= TRACK_SCORE_CUT ) sel_has_non_proton_particles_ = true;
     ...
     // }
      ...
      v3p = v3p.Unit() * p_mom;
      if(p_mom > 0.25 && p_mom < 1.2){
        proton_index.push_back(p);
        proton_e.push_back(KEp + PROTON_MASS);
        proton_px.push_back(v3p.X());
        proton_py.push_back(v3p.Y());
        proton_pz.push_back(v3p.Z());
      }
  ...
  for(int i_num_p = 0; i_num_p < sel_num_proton_candidates; i_num_p++){
    sel_proton_candidate_index[i_num_p] = proton_index[i_num_p];
    sel_proton_candidate_e[i_num_p] = proton_e[i_num_p];
    sel_proton_candidate_px[i_num_p] = proton_px[i_num_p];
    sel_proton_candidate_py[i_num_p] = proton_py[i_num_p];
    sel_proton_candidate_pz[i_num_p] = proton_pz[i_num_p];
  }
...
//  }
  ...
  // candidate)
  bool sel_CCXp0pi_ = sel_nu_mu_cc_ && sel_no_reco_showers_
...
EventCategory CC1muXp0pi::CategorizeEvent(AnalysisEvent* Event) {
