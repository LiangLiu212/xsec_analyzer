 ...
 *
 *    Description:  Collection of code from Steven's res_plots.cxx and make_config_all.cxx
 *                  This is intended to help with choosing binning for kinematic variables
 *                  and building the configuration files of binning and slice.
 *
 *        Version:  1.0
...
void MakeConfig::ResPlots(){
  ...
  for(int i = 0; i < vect_block.size(); i++){
    ...
    if(vect_block.at(i).block_true_->Is1D()){
      make_res_plots(vect_block[i].block_reco_->GetXName().Data(), 
          vect_block[i].block_reco_->GetXTitle().Data(), 
          vect_block[i].block_reco_->GetSelection().Data(),
          RUNS,
          vect_block[i].block_reco_->GetVector(), 
          false, false, 
          DEFAULT_TRUE_BINS,
          vect_block[i].block_true_->GetXName().Data(), 
          vect_block[i].block_true_->GetSelection().Data(), 
      ...
      for(int j = 0; j < vect_block.at(i).block_true_->GetNBinsX(); j++){
        ...
        for(int k = 0; k < vect_block.at(i).block_true_->GetNBinsY(j); k++){
          true_bins.emplace_back(vect_block.at(i).block_true_->GetBinDef(j, k).Data(), TrueBinType(vect_block.at(i).block_true_->GetBinType()), i);
          reco_bins.emplace_back(vect_block.at(i).block_reco_->GetBinDef(j, k).Data(), RecoBinType(vect_block.at(i).block_reco_->GetBinType()), i);
        }
      }

      TString temp_file = "/exp/uboone/app/users/liangliu/analysis-code/xsec_analyzer_liang/Configs/temp_bin_config.txt";

      std::ofstream out_file( temp_file.Data() );
      out_file << "temp\n";
      out_file << "stv_tree\n";
      ...
      reco_bins.clear();
      out_file.close();
      make_res_plots(temp_file.Data(), RUNS);
...
void MakeConfig::Print(){
  ...
  for(int i = 0; i < vect_block.size(); i++){
    ...
    if(vect_block.at(i).block_true_->Is1D()){
      ...
      sb.slice_vars_.emplace_back(vect_block.at(i).block_true_->GetXTitle().Data(),
          vect_block.at(i).block_true_->GetXTexTitle().Data(),
          vect_block.at(i).block_true_->GetXTitleUnit().Data(), 
          vect_block.at(i).block_true_->GetXTexTitleUnit().Data());
      ...
      for(int j = 0; j < vect_block.at(i).block_true_->GetNBinsX(); j++){
        ...
        true_bins.emplace_back( vect_block.at(i).block_true_->GetBinDef(j).Data(),  TrueBinType(vect_block.at(i).block_true_->GetBinType()), i);

      }
      for(int j = 0; j < vect_block.at(i).block_reco_->GetNBinsX(); j++){
        reco_bins.emplace_back( vect_block.at(i).block_reco_->GetBinDef(j).Data(), RecoBinType(vect_block.at(i).block_reco_->GetBinType()), i);
      }
    }
    else{
      sb.slice_vars_.emplace_back(vect_block.at(i).block_true_->GetXTitle().Data(),
      ...
      sb.slice_vars_.emplace_back(vect_block.at(i).block_true_->GetYTitle().Data(),
          vect_block.at(i).block_true_->GetYTexTitle().Data(), 
          vect_block.at(i).block_true_->GetYTitleUnit().Data(), 
          vect_block.at(i).block_true_->GetYTexTitleUnit().Data());
  ...
  for(const auto& bkg_idx : background_index){
    TString bdef = CATEGORY + Form(" == %d", bkg_idx);
    true_bins.emplace_back(bdef.Data(), kBackgroundTrueBin, DUMMY_BLOCK_INDEX);
  }
...
void MakeConfig::make_res_plots( const std::string& rmm_config_file_name,
    const std::set<int>& runs,
    const std::string& universe_branch_name,
    size_t universe_index,
    bool show_smear_numbers)
{
  ...
  std::cout <<  universe_key << " : " << universe_index << std::endl;
...
void Block1D::Init(){
  ...
  this->SetTexTitle(fTexTitle.Data());
...
void Block1D::SetTexTitle(const char *textitle){
  fTexTitle = textitle;
  fTexTitle.ReplaceAll("#;",2,"#semicolon",10);
  std::vector<TString> str_container;
...
void Block2D::SetTexTitle(const char *textitle){
  fTexTitle = textitle;
  fTexTitle.ReplaceAll("#;",2,"#semicolon",10);
  std::vector<TString> str_container;
  ...
  Int_t lns = str1.Length();
  if(isc < 0) return;
  //   throw std::runtime_error("Wrong title -> " + fTexTitle + ". The format of the title of 2D block must be <branch title>; <unit>; <y branch title>; <y unit>.");
  while(isc >=0){
    str_container.push_back(str1(0,isc));
    str1 = str1(isc+1, lns);
    isc = str1.Index(";");
